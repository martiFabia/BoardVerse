...
public interface UserNeo4jRepository extends Neo4jRepository<UserNeo4j, String> {
    ...
    /**
     * Creates a user, if the new username is not already taken.
     * @param oldUsername the username of the user
     * @param newUsername the username of the user
     * @return true if the user was created, false otherwise
     */
    @Query("MATCH (u:User {username: $oldUsername}) " +
            "OPTIONAL MATCH (existing:User {username: $newUsername}) " +
            "WITH u, existing " +
     ...
     * @return true if the user participated in the tournament, false otherwise
     */
    @Query("""
     ...
     */
    @Query("MATCH (u:User {username: $username})-[p:PARTICIPATES]->(t:Tournament)-[:IS_RELATED_TO]->(g:Game) " +
        "RETURN " +
        "   CASE " +
        "       WHEN t.visibility <> 'PUBLIC' AND  $username <> $currentUsername" +
        "       THEN null " +
     ...
     */
    @Query("MATCH (u:User {username: $username})-[w:WON]->(t:Tournament)-[:IS_RELATED_TO]->(g:Game) " +
        "RETURN " +
        "   CASE " +
        "       WHEN t.visibility <> 'PUBLIC' AND  $username <> $currentUsername" +
        "       THEN null " +
    ...
    List<UserSuggestionDTO> getUsersRecommendation(String username, int pageSize, int pageNumber);
    ...
    @Query("""
            ...
            LIMIT $pageSize
    """)
    List<UserSuggestionDTO> findGameSimilarities(@Param("username") String username, @Param("pageSize") int pageSize, @Param("pageNumber") int pageNumber);
